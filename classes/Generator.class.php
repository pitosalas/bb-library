...
//
// $Id: Generator.class.php,v 1.2 2007/08/17 15:16:33 alg Exp $
//
...
$stop_words = array("a's", "able", "about", "above", "according",
	...
	"hopefully", "how", "howbeit", "however", "i'd", "i'll", "i'm", "i've", 
	"ie", "if", "ignored", "immediate", "in", "inasmuch", "inc", "indeed", 
	"indicate", "indicated", "indicates", "inner", "insofar", "instead", "into", 
	"inward", "is", "isn't", "it", "it'd", "it'll", "it's", "its", "itself", 
	"just", "keep", "keeps", "kept", "know", "knows", "known", "last", "lately", 
	...
	"sub", "such", "sup", "sure", "t's", "take", "taken", "tell", "tends", "th", 
	...
	"usually", "value", "various", "very", "via", "viz", "vs", "want", "wants", 
...
/**
 * Generates tags and descriptions for items and folders.
 */
class Generator
{
	/** Generates tags for a given title and merges them with other list. */
	function tags($title, $merge_with = false)
	{
		$title = strtolower($title);
		$title = ereg_replace("[^a-z]", ' ', $title);
		...
		$tags = Generator::_remove_stop_words($tags);
		if ($merge_with)
		{
			foreach ($merge_with as $v)
			{
	...
	function _remove_stop_words($tags)
	{
		global $stop_words;
		return array_values(array_diff($tags, $stop_words));
	}
	
	/** Generates the description for a folder by its name and the tags of its owner. */
	function folder_description($title, $owner_name, $owner_tags)
	{
		$desc = null;
		
		if (in_array('expert', $owner_tags) && strlen(trim($title)) > 0)
		{
			$desc = "$owner_name's recommendations for the best feeds and blogs about: $title";
		} else if (in_array('publisher', $owner_tags))
		{
			$desc = "A collection of feeds and blogs from $owner_name";
		}
